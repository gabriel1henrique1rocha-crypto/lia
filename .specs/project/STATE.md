...
- [ ] **AÇÃO PENDENTE (produção) — aplicar migration `0006_genre_public_read.sql`**: NÃO foi aplicada em prod (só local). Aplicar via `supabase db push` **após revisão**. Sem ela, o filtro de gênero fica vazio e a ficha da review-page omite o gênero (ver achado abaixo).
- [ ] **ACHADO (lacuna pré-existente):** `genre` tinha RLS habilitada (0001) + GRANT (0004) mas **nenhuma policy de SELECT** (0003 só cobriu `book`) → anon lia 0 gêneros. Afeta também a **review-page em produção** (ficha exibe gênero **null/omitido** hoje). Corrigido pela **0006** (aprovada como exceção pontual ao "sem migration" do LST-19). Verificar a ficha em prod após aplicar a 0006.
- [ ] **PRÓXIMO PASSO — revisar a branch `feat/review-listing-search` e, se aprovada, abrir PR + aplicar 0006 em prod.**
