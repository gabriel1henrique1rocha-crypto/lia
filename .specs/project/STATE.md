...
- [ ] **Flag pré-Execute (decisão, não bloqueia):** confirmar adoção das cores **sálvia** (T3/T5) — DD-7 já adotou `sage-100/700` com contraste 13.0:1/5.5:1; fecha a pendência de STATE "cores sálvia". Alternativa: destaque sem sálvia (paper/oxblood existentes).
- [ ] **PRÓXIMO PASSO — `review-listing-search` Execute** (após OK do tasks.md)
